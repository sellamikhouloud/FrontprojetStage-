...
const { user } = useAuth();
  ...
  // ==================== RÉCUPÉRATION DES DISTRIBUTIONS (BACKEND) ====================
  const {
    ...
    queryFn: () => listDistributions().then((r) => r.data),
    // Si le backend accepte des query params de dates, on peut les passer ici :
    // queryFn: () =>
    //   listDistributions({
    //     date_debut: appliedFilters.dateDebut
    //       ? appliedFilters.dateDebut.toISOString().slice(0, 10)
    //       : undefined,
    //     date_fin: appliedFilters.dateFin
    //       ? appliedFilters.dateFin.toISOString().slice(0, 10)
    //       : undefined,
    //   }).then((r) => r.data),
  });
...
const isAdmin = role === "admin";
  ...
  // ==================== FILTRAGE (recherche + dates) SUR DONNÉES RÉELLES ====================
  const filtered = distributionsData.filter((item) => {
    ...
    // date_distribution vient au format "YYYY-MM-DD"
  ...
  // Remplace l'ancien parsing de strings ("5 kg") par une lecture directe des vrais objets produit.
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-3">
              {filtered.map((item, index) => {
                const enfantPrenom = item.famille?.nourrisson?.prenom ?? "";
                const mereNom = item.famille?.mere?.nom ?? "";
                const nomAffiche = `${enfantPrenom} `.trim() || "-";
